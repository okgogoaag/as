...
-- THIS IS THE REDECLARED TABLE, SHOULD PROBABLY BE RENAMED TO AVOID CONFUSION
local LibraryTable = {} -- Renamed from WindowTable to avoid conflict with the one above

function LibraryTable:Window(Htitle,GMtitle,toclose)
	...
	function Tabs:Tab(tabname)
		...
		-------------------------------------------------------
		--               NEW INPUT FUNCTION ADDED HERE               --
		-------------------------------------------------------
		function ElementHandler:Input(title, options)
			...
			InputFrame.Parent = NewPages
			InputFrame.BackgroundColor3 = Color3.fromRGB(29, 26, 53)
			InputFrame.BorderSizePixel = 0
			InputFrame.Size = UDim2.new(0, 375, 0, 32)
			...
			InputTitle.Position = UDim2.new(0.037, 0, 0, 0)
			InputTitle.Size = UDim2.new(0.45, 0, 1, 0) -- Give it about half the space
			InputTitle.Font = Enum.Font.Gotham
			InputTitle.Text = options.Title or title
			InputTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
			InputTitle.TextSize = 14.000
			InputTitle.TextXAlignment = Enum.TextXAlignment.Left

			-- Configure the actual text box input
			InputField.Name = "InputField"
			InputField.Parent = InputFrame
			InputField.BackgroundColor3 = Color3.fromRGB(74, 68, 94)
			InputField.Position = UDim2.new(0.5, 0, 0.156, 0)
			InputField.Size = UDim2.new(0.47, 0, 0.7, 0)
			...
			InputField.Text = options.Default or ""
			InputField.PlaceholderText = options.Placeholder or ""
			...
			InputObject.Value = InputField.Text
			function InputObject:OnChanged(func)
				if type(func) == "function" then
					table.insert(OnChangedEvents, func)
				end
			end
			
			-- Internal function to fire all OnChanged listeners
			local function FireOnChanged()
				InputObject.Value = InputField.Text
				for _, func in ipairs(OnChangedEvents) do
					pcall(func, InputObject.Value)
				end
			end

			-- Handle Text Changes
			InputField:GetPropertyChangedSignal("Text"):Connect(function()
				local currentText = InputField.Text
				
				-- Handle numeric-only input
				if options.Numeric then
					local filteredText = currentText:gsub("%D", "") -- Remove all non-digits
					if currentText ~= filteredText then
						InputField.Text = filteredText
						-- The change event will fire again, so we exit here
						return
					end
				end

				-- Fire OnChanged for every update
				FireOnChanged()

				-- If "Finished" is false, call the main callback on every change
				if not options.Finished then
					pcall(callback, InputField.Text)
				end
			end)

			-- Handle when user presses Enter or clicks away
			InputField.FocusLost:Connect(function(enterPressed)
				-- If "Finished" is true, only call the main callback when Enter is pressed
